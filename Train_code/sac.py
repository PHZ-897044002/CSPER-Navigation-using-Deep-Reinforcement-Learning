...
class SAC():
    ...
    def soft_update(self, TAU=0.01):
        with torch.no_grad():
            ...
            ###########################################################################################
            for targetParam, evalParam in zip(self.actor_target.parameters(), self.actor.parameters()):
                targetParam.copy_((1 - self.tau) * targetParam.data + self.tau * evalParam.data)
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            #########################################################
            a_next, logpi_next, _ = self.actor_target.sample(sn_ts)
            ############################################################################
            # q_next_target = self.critic_target(sn_ts, a_next) - self.alpha * logpi_next
            q1_next_target, q2_next_target = self.critic_target(sn_ts, a_next)
            q_next_target = torch.min(q1_next_target, q2_next_target) - self.alpha * logpi_next
        ...
        q1, q2 = self.critic(s_ts, a_ts)
        TD_errors1 = q_target - q1
        q1_loss = 0.5*F.mse_loss(TD_errors1, torch.zeros_like(TD_errors1))
        TD_errors2 = q_target - q2
        q2_loss = 0.5*F.mse_loss(TD_errors2, torch.zeros_like(TD_errors2))
        self.critic_loss = q1_loss + q2_loss
        # TD_errors = q_target - q_eval
        # self.critic_loss = F.mse_loss(TD_errors, torch.zeros_like(TD_errors))
        ###############################################################################
        ...
        ####################################################################
        a_curr, logpi_curr, _ = self.actor.sample(s_ts)
        q1_a, q2_a = self.critic(s_ts, a_curr)
        min_q_a = torch.min(q1_a, q2_a)
        self.actor_loss = ((self.alpha * logpi_curr) - min_q_a).mean()
        #####################################################################
